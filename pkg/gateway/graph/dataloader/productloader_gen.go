...
type ProductLoaderConfig struct {
	...
	Fetch func(keys []uuid.UUID) ([]*model.Product, []error)
...
type ProductLoader struct {
	...
	cache map[uuid.UUID]*model.Product
...
func (l *ProductLoader) Load(key uuid.UUID) (*model.Product, error) {
...
func (l *ProductLoader) LoadThunk(key uuid.UUID) func() (*model.Product, error) {
	l.mu.Lock()
...
func (l *ProductLoader) LoadAll(keys []uuid.UUID) ([]*model.Product, []error) {
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *ProductLoader) LoadAllThunk(keys []uuid.UUID) func() ([]*model.Product, []error) {
	results := make([]func() (*model.Product, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *ProductLoader) Prime(key uuid.UUID, value *model.Product) bool {
	l.mu.Lock()
	var found bool
...
func (l *ProductLoader) unsafeSet(key uuid.UUID, value *model.Product) {
	if l.cache == nil {
		l.cache = map[uuid.UUID]*model.Product{}
	}
	l.cache[key] = value
}
